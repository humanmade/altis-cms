...
offered by WordPress as convenience functions.
...
For the most part, `whitelist_html`
can be used in exactly the same way developers are used to using other escaping
functions. 

**`whitelist_html( string $text, array $allowedtags = array(), string $context = '' ) : string`**
...
It works with multiple elements as well, using a
...
Read the [official documentation](https://github.com/humanmade/whitelist-html) for rationale and more information.
